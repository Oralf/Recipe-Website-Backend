...
async function getInfoOnRecipesForUser(user_id, recipeIDs_arr) {
    ...
    try {
        ...
        for (i = 0; i < recipeIDs_arr.length; i++) {
            ...
            if (favorite_recipes_for_user && recipeInFavorites(recipe, favorite_recipes_for_user)) {
                curr_favorite = true;
            }
            if (watched_recipes_for_user && recipeInWatch(recipe, watched_recipes_for_user)) {
                curr_watched = true;
            }
...
async function getAllFavortieRecipeID(user_id) {
    try {
        ...
        for (j = 0; j < favorite_recipes.length; j++) {
...
function recipeInFavorites(recipeID, favorite_recipes_arr) {
    for (j = 0; j < favorite_recipes_arr.length; j++) {
...
function recipeInWatch(recipeID, watched_arr) {
    for (j = 0; j < watched_arr.length; j++) {
