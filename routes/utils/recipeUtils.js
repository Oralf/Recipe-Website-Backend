...
// check if all the random recipes contains all information
 function recipeConatinAllFields(arrayOfRandomRecipes){
  ...
  if(recipe.analyzedInstructions== "" || !recipe.instructions ||recipe.extendedIngredients== "" || !recipe.extendedIngredients){
...
async function getFullRecipesInfo(id) {
  let fullRecipeFromApi = await axios.get(`${recipes_api_url}/${id}/information?${api_key}`);
...
function makeFullRecipe(recipe){
  ...
  let arrInstruction=[];
  analyzedInstructions.map((crrStep) =>
  arrInstruction.push(crrStep.step))
  // add ingredients
